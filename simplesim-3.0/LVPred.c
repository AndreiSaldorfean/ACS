...
int foundAssociativeLVPTAddress(md_addr_t addr, sword_t value, int history)
{
    ...
    if(lvpt == NULL) // nu exista nici o instructiune adaugata tabelei de predictie
        return found;

    if(p->addr == addr) // instructiunea a fost adaugata anterior în tabela de predictie
    {
        found = 1;
        if(p->values != NULL) // la adresa respectiva a existat cel putin o valoare
        {
            if(value == predictValue(p, history))
            {
                if(p->automat == 2 || p->automat == 3) /* automatul se afla în starea predictibil */
                {
                    valuePrediction++;
                    classifiedPred++;
                    predictable++;
