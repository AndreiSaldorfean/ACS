...
#if 1 /* NOTE: THESE SHOUDLN'T BE HERE */
...
static counter_t Tacces_DRAM = 0;
    ...
    else if (history > 1 && q != NULL) // contextual prediction
    {
        ...
        for (i = 0; i < history - pattern; i++)
        {
            ...
            if (isPattern)
            {
                ...
                {
                    newValue =
                        (LVPTvalueList)malloc(sizeof(struct LVPTelement));
                    newValue->value =
                        q->value; // the value which follows the pattern
                    newValue->count = 1;
...
void sim_reg_options(struct opt_odb_t *odb)
{
    ...
    opt_reg_uint(
        odb,
        "-memaddr",
        "address type {0-instruction address| 1-memory address}",
        &memAddress,
        /* default */ 0,
        /* print */ TRUE,
        /* format */ NULL);
    ...
    opt_reg_uint(
        odb,
        "-pred",
        "what to do {0-load value locality| 1-prediction}",
        &predict,
        /* default */ 0,
        /* print */ TRUE,
        /* format */ NULL);
    ...
    opt_reg_uint(
        odb,
        "-assoc",
        "table type {0-dirrect mapped| 1-associative}",
        &isAssoc,
        /* default */ 1,
        /* print */ TRUE,
        /* format */ NULL);
    ...
    opt_reg_uint(
        odb,
        "-history",
        "maximum number of values memorized for a certain address",
        &history,
        /* default */ 1,
        /* print */ TRUE,
        /* format */ NULL);
