...
COMMON="-fastfwd 300000000 -max:inst 1000000000 -cache:dl1 dl1:256:64:2:l -cache:dl1lat 2 -cache:il1 il1:512:8:8:l -cache:il1lat 2 -cache:dl2 ul2:4096:256:2:l -cache:dl2lat 5 -cache:il2lat 5 -mem:lat 270 4 -issue:width 8 -commit:width 32 -decode:width 8 -lsq:size 64 -rob:size 256 -iq:size 64 -rf:size 128 -res:ialu 8 -res:imult 8 -res:fpalu 2 -res:fpmult 4 -lvpt:size 128 -lvpt:memaddr 0 -lvpt:assoc 8 -lvpt:history 2 -lvpt:access 1"
...
run_sim() {
    ...
    cd "$workdir"
    "$BASEDIR/sim-outorder" -redir:sim "$BASEDIR/results/${name}_with_lvpt_c2.res" $COMMON "$argfile"
    mv "$workdir/results/powerfile.ptrace" "$BASEDIR/results/${name}_with_lvpt_c2.ptrace"
    cd "$BASEDIR"
    rm -rf "$workdir"
}
