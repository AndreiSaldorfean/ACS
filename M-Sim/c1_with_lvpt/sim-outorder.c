...
struct LVPTLocation
{
  ...
  byte_t counter[4];	// 2-bit saturating counter(s)
  qword_t value[4];	// Load value(s)
  byte_t eLRU;		// LRU counter initialized on access with (assoc-1), decremented when other locations are accessed
  byte_t vLRU[4];	// One LRU counter for each Value. Initialized on access with (LVPT_history-1), decremented when other locations are accessed
};

/* LVPT declaration */
int LVPT_size = 256;
struct LVPTLocation lvpt[10][10000];	// maximum 10 threads and 10000 entries per thread
int memAddress;  			// 0-instruction address (PC); 1-memory address;
int LVPT_assoc = 1;			// direct mapped
int LVPT_history = 1;			// last value predictor
int LVPT_access_type = 1;		// accessing the LVPT only on miss in DL1
int LVPT_lat;				// will keep the LVPT latency - computed in sim_main

/* Insert a new Load into the LVPT */
/* Instead of qword_t could be also dfloat_t, both types being on 64 bits */
void addToLVPT(int context_id, unsigned int index, md_addr_t tag, qword_t val, int history)
{
  int i;
  lvpt[context_id][index].tag = tag;
  lvpt[context_id][index].counter[0] = 0;	// it's the first value for this load
  lvpt[context_id][index].value[0] = val;	// it's the first value for this load
  lvpt[context_id][index].vLRU[0] = history-1;	// it's the first value for this load
  for(i=1; i<history; i++)			// reseting the other counters and LRUs
  {
    lvpt[context_id][index].counter[i] = 0;
    lvpt[context_id][index].vLRU[i] = 0;
  }
}
 ...
 */
bool_t isInLVPT(int context_id, unsigned int index, md_addr_t tag)
{
  if(lvpt[context_id][index].tag == tag) return TRUE;
  return FALSE;
}

/*
 * selects the value with the highest confidence
 */
unsigned int selectValueIndex(int context_id, unsigned int index, int history)
{
  unsigned int i;
  unsigned int selected_index = 0;
  for(i=1; i<history; i++)
	if(lvpt[context_id][index].counter[i] > lvpt[context_id][index].counter[selected_index])
		selected_index = i;
 ...
 */
bool_t isPredictable(int context_id, unsigned int index, unsigned int selected_value_index)
{
  if(lvpt[context_id][index].counter[selected_value_index] > 1) return TRUE;
  return FALSE;
}
 ...
 * Instead of qword_t could be also dfloat_t, both types being on 64 bits.
 */
bool_t correctPrediction(int context_id, unsigned int index, qword_t val, unsigned int selected_value_index)
{
  if(lvpt[context_id][index].value[selected_value_index] == val) return TRUE;	
  return FALSE;
}
 ...
 * Instead of qword_t could be also dfloat_t, both types being on 64 bits.
 */
void updateLVPT(int context_id, unsigned int index, /*md_addr_t tag, */qword_t val, /*bool_t increase_counter, */unsigned int history)
{
	int i;
	int accessed_value_index = 0;
	bool_t found = FALSE;
	//lvpt[context_id][index].tag = tag;
	//lvpt[context_id][index].value = val;
	for(i=0; i<history; i++)
	{
		if(lvpt[context_id][index].value[i] == val){
			found = TRUE;
			accessed_value_index = i;
			if(lvpt[context_id][index].counter[i] < 3) lvpt[context_id][index].counter[i]++;
		}
		else if(lvpt[context_id][index].value[i] != val && (lvpt[context_id][index].counter[i] > 0)) 
			lvpt[context_id][index].counter[i]--;
	}
	if(!found)
	{
		for(i=1; i<history; i++)
			if(lvpt[context_id][index].vLRU[i] < lvpt[context_id][index].vLRU[accessed_value_index])
				accessed_value_index = i;
		lvpt[context_id][index].value[accessed_value_index] = val;
		/* all the counters, including that from "accessed_value_index", were decremented in the previous "for" */
		//lvpt[context_id][index].counter[accessed_value_index] = 0;
	}
	for(i=0; i<history; i++)
	{
		if(i==accessed_value_index) lvpt[context_id][index].vLRU[i] = history-1;
		else if(lvpt[context_id][index].vLRU[i]>0) lvpt[context_id][index].vLRU[i]--;
	}
	
}

/* 
 * Select the Least Recently Used location within a given set
 */
unsigned int selectLRU(int context_id, unsigned int k)
{
	unsigned int i;
	unsigned int selected = k;
	for(i=k; i < k + LVPT_assoc; i++)
		if(lvpt[context_id][i].eLRU == 0){
			selected = i;
			break;
		}
	return selected;
...
/*
 * Update all LRU counters within a given set
 */
void updateLRU(int context_id, unsigned int k, unsigned int accessed_LVPT_index)
{
	unsigned int i;
	for(i=k; i < k + LVPT_assoc; i++){
		if(i == accessed_LVPT_index)
			lvpt[context_id][i].eLRU = LVPT_assoc - 1;
		else if(lvpt[context_id][i].eLRU > 0)
			lvpt[context_id][i].eLRU--;
	}
}
...
static long long INF = 9999999999;
...
"  Predictor `comb' combines a bimodal and a 2-level predictor.\n"
  ...
  opt_reg_int(odb, "-lvpt:size",
	      "LVPT size",
	      &LVPT_size, /* default */256,
	      /* print */TRUE, /* format */NULL);

  /* choosing instruction address or memory address @ Arpad Gellert */
  opt_reg_int(odb, "-lvpt:memaddr",
	      "address type {0-instr. address| 1-memory address}",
	      &memAddress, /* default */0,
	      /* print */TRUE, /* format */NULL);

  /* LVPT associativity @ Arpad Gellert */
  opt_reg_int(odb, "-lvpt:assoc",
	      "LVPT associativity {1-direct-mapped| 2-two-way| 4-four-way| etc}",
	      &LVPT_assoc, /* default */1,
	      /* print */TRUE, /* format */NULL);


  /* the number of values used as history in LVPT @ Arpad Gellert */
  opt_reg_int(odb, "-lvpt:history",
	      "LVPT history {1 <= h <= 4}",
	      &LVPT_history, /* default */1,
	      /* print */TRUE, /* format */NULL);

  /* accessing the LVPT on miss in DL1 or on miss in DL2 @ Arpad Gellert */
  opt_reg_int(odb, "-lvpt:access",
	      "LVPT access type {1 = on miss in DL1| 2 = on miss in DL2}",
	      &LVPT_access_type, /* default */1,
	      /* print */TRUE, /* format */NULL);
...
struct ROB_entry {
  ...
  int exec_cycle;			/* added by A. Gellert to keep real latency of load */
  bool_t predicted;			/* added by A. Gellert and means correctly predicted load */
...
commit(int context_id)
{
  ...
  while (contexts[context_id].ROB_num > 0 && committed < commit_width)
      ...
      if (contexts[context_id].ROB[contexts[context_id].ROB_head].ea_comp)
	{
	  ...
	  /* by A. Gellert - the predicted load must wait the real value for verification */
	  if(contexts[context_id].LSQ[contexts[context_id].LSQ_head].predicted && contexts[context_id].LSQ[contexts[context_id].LSQ_head].exec_cycle > sim_cycle)
 	    break;
	      ...
	      if (MD_OP_FUCLASS(rs->op) != NA)
		{
		  ...
		  if (fu)
			 ...
			 */
			  ...
			  if((LVPT_access_type == 1 && load_lat > cache_dl1_lat) || (LVPT_access_type == 2 && load_lat > cache_dl2_lat))
			  {
				md_addr_t load_address;
				if(memAddress) load_address = rs->addr >> 2;
				else load_address = rs->PC >> 2;
				unsigned int LVPT_index;
				unsigned int LVPT_set = load_address & ((LVPT_size / LVPT_assoc) - 1);
				md_addr_t LVPT_Tag = load_address >> ((int)log2(LVPT_size) - LVPT_assoc + 1);
				bool_t LVPT_hit = FALSE;
				unsigned int selected_LVPT_index;
				unsigned int selected_value_index;
				lvpt_access++;	/* access for read */
				n_critical_loads++;
				/* associative search within the set */
				for(LVPT_index = LVPT_assoc*LVPT_set; LVPT_index < LVPT_assoc*(LVPT_set+1); LVPT_index++){
				    /* hit in LVPT */
				    if(isInLVPT(rs->context_id, LVPT_index, LVPT_Tag)){
					selected_LVPT_index = LVPT_index;
					/* selecting the value with the highest confidence */
					selected_value_index = selectValueIndex(rs->context_id, LVPT_index, LVPT_history);
					/* the counter is in "predict" state */
					if(isPredictable(rs->context_id, LVPT_index, selected_value_index)){
						n_load_predictions++;
						/* prediction & update */
						if(correctPrediction(rs->context_id, LVPT_index, rs->val_ra, selected_value_index)){
							/* Correct Prediction */
							n_correct_load_predictions++;
							// updateLVPT(rs->context_id, LVPT_index, /*LVPT_Tag, */rs->val_ra, /*TRUE*/LVPT_history);
							if(rs->LSQ_index != -1){
								/* Correct prediction flag used in the Commit stage */
      								contexts[rs->context_id].LSQ[rs->LSQ_index].predicted = TRUE;
								/* Real execution latency, verified in the Commit stage */
								contexts[rs->context_id].LSQ[rs->LSQ_index].exec_cycle = sim_cycle + contexts[rs->context_id].LSQ[rs->LSQ_index].exec_lat;
							}    
							/* Correctly predicted load's latency */
							rs->exec_lat = (LVPT_access_type==1 ? cache_dl1_lat : cache_dl2_lat) + LVPT_lat;
						}
						else{
							/* Misprediction */
							// updateLVPT(rs->context_id, LVPT_index, /*LVPT_Tag, */rs->val_ra, /*FALSE*/LVPT_history);
							/* Mispredicted load's additional latency (recovery)*/
							rs->exec_lat += 20;	
						}
					}
					...
					// else{
						...
						// if(correctPrediction(rs->context_id, LVPT_index, rs->val_ra, selected_value_index)) updateLVPT(rs->context_id, LVPT_index, /*LVPT_Tag, */rs->val_ra, /*TRUE*/LVPT_history);
						// else updateLVPT(rs->context_id, LVPT_index, /*LVPT_Tag, */rs->val_ra, /*FALSE*/LVPT_history);
					// }
					updateLVPT(rs->context_id, LVPT_index, /*LVPT_Tag, */rs->val_ra, /*FALSE*/LVPT_history);
					LVPT_hit = TRUE;
					break;
				...
				/* miss in LVPT */
				if(!LVPT_hit){
				    unsigned int LVPT_index = selectLRU(rs->context_id, LVPT_assoc*LVPT_set);
				    addToLVPT(rs->context_id, LVPT_index, LVPT_Tag, rs->val_ra, LVPT_history);
				    selected_LVPT_index = LVPT_index;
				}
				updateLRU(rs->context_id, LVPT_assoc*LVPT_set, selected_LVPT_index);
				lvpt_access++; /* access for update */
			  }
...
#include "machine.def"
      ...
      if (op != MD_NOP_OP)
	{
	  ...
	  rs = &contexts[disp_context_id].ROB[contexts[disp_context_id].ROB_tail];
          rs->slip = sim_cycle - 1;
	  rs->predicted = FALSE;	/* by A. Gellert */
	  rs->IR = inst;
	  rs->op = op;
	  rs->PC = regs->regs_PC;
	  ...
	  if (MD_OP_FLAGS(op) & F_MEM)
	    {
	      ...
	      lsq->predicted = FALSE;	/* by A. Gellert */
	      lsq->IR = inst;
	      lsq->op = op;
	      lsq->PC = regs->regs_PC;
...
sim_main(void)
{
  ...
  /*
   * Computing LVPT latency based on CACTI 5.3 evaluations
   * @author Arpad Gellert, ULBS, 26/11/2012
   */
  LVPT_lat = 2;	// default latency - majority of cases
  if(LVPT_size <= 4096 && LVPT_assoc == 1 && LVPT_history == 1) LVPT_lat = 1;
  if(LVPT_size <= 1024 && LVPT_assoc == 1 && LVPT_history == 2) LVPT_lat = 1;
  if(LVPT_size <= 512 && LVPT_assoc == 1 && LVPT_history == 4) LVPT_lat = 1;
  if(LVPT_size == 8192 && LVPT_assoc == 8 && LVPT_history == 2) LVPT_lat = 3;
  if(LVPT_size == 8192 && LVPT_assoc >=2 && LVPT_history == 4) LVPT_lat = 3;
  if(LVPT_assoc == 8 && LVPT_history == 4) LVPT_lat = 3;
...
#include "machine.def"
  ...
  /* 
   * Create the file for dumping the power measurements (power trace)
   * @author Claudiu-Raul Buduleci, ULBS, 7/11/2012
   */
  FILE *fp_pwr_trace;
  fp_pwr_trace = fopen("results/powerfile.ptrace", "w");
  fprintf(fp_pwr_trace, "IntExec    L2    Dcache     L2_right    DTB    LdStQ    Bpred    FPReg    FPExec    Rename    LVPT    IntReg    ITB    InstQ    L2_left    Icache\n");
  fclose(fp_pwr_trace);
  ...
  for (;;)
	...
	for(i=0;i<num_contexts;i++)
      ...
      /* 
      * Dump the power measurements (power trace) every 500 kilo-cycle
      * @author Claudiu-Raul Buduleci, ULBS, 7/11/2012
      */
      if((sim_cycle % 500000) == 0) {
        print_power_trace(sim_cycle);
      }
