...
COMMON="-fastfwd 300000000 -max:inst 1000000000 -cache:dl1 dl1:64:64:2:l -cache:dl1lat 2 -cache:il1 il1:512:64:1:l -cache:il1lat 2 -cache:dl2 ul2:1024:128:8:l -cache:dl2lat 6 -cache:il2lat 6 -mem:lat 270 4 -issue:width 4 -commit:width 32 -decode:width 8 -lsq:size 64 -rob:size 128 -iq:size 128 -rf:size 64 -res:ialu 8 -res:imult 8 -res:fpalu 2 -res:fpmult 4 -lvpt:size 8192 -lvpt:memaddr 0 -lvpt:assoc 8 -lvpt:history 2 -lvpt:access 1"
...
run_sim() {
    ...
    cd "$workdir"
    "$BASEDIR/sim-outorder" -redir:sim "$BASEDIR/results/${name}_with_lvpt_c1.res" $COMMON "$argfile"
    mv "$workdir/results/powerfile.ptrace" "$BASEDIR/results/${name}_with_lvpt_c1.ptrace"
    cd "$BASEDIR"
    rm -rf "$workdir"
}
