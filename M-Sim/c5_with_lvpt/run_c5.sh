...
COMMON="-fastfwd 300000000 -max:inst 1000000000 -cache:dl1 dl1:256:256:2:l -cache:dl1lat 3 -cache:il1 il1:32:256:8:l -cache:il1lat 9 -cache:dl2 ul2:4096:256:8:l -cache:dl2lat 10 -cache:il2lat 10 -mem:lat 270 4 -issue:width 16 -commit:width 32 -decode:width 32 -lsq:size 512 -rob:size 1024 -iq:size 256 -rf:size 256 -res:ialu 8 -res:imult 8 -res:fpalu 8 -res:fpmult 8 -lvpt:size 8192 -lvpt:memaddr 0 -lvpt:assoc 4 -lvpt:history 2 -lvpt:access 1"
...
run_sim() {
    ...
    cd "$workdir"
    "$BASEDIR/sim-outorder" -redir:sim "$BASEDIR/results/${name}_with_lvpt_c5.res" $COMMON "$argfile"
    mv "$workdir/results/powerfile.ptrace" "$BASEDIR/results/${name}_with_lvpt_c5.ptrace"
    cd "$BASEDIR"
    rm -rf "$workdir"
}
